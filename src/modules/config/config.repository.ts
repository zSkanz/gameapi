...
export function validateEntry(key: string, input: { type: unknown; value: unknown; description?: unknown }): ConfigEntry {
  ...
  if (typeof description !== 'string' || description.length > CONFIG_LIMITS.maxDescriptionLength) {
  ...
  switch (type) {
    ...
    case 'json':
      ...
      if (JSON.stringify(value).length > CONFIG_LIMITS.maxJsonLength) throw bad(`JSON is limited to ${CONFIG_LIMITS.maxJsonLength.toLocaleString('en-US')} characters.`);
      break;
    default:
 ...
 */
const cachesByPool = new WeakMap<Pool, Map<string, { at: number; value: PublishedValues }>>();

export class ConfigRepository {
  private readonly valuesCache: Map<string, { at: number; value: PublishedValues }>;

  constructor(private readonly pg: Pool) {
    let cache = cachesByPool.get(pg);
    if (!cache) cachesByPool.set(pg, (cache = new Map()));
    this.valuesCache = cache;
  }

  /** The hot read. Cached in-process briefly; a publish on this worker clears it immediately. */
  async values(gameId: string): Promise<PublishedValues> {
    const hit = this.valuesCache.get(gameId);
    if (hit && Date.now() - hit.at < VALUES_CACHE_MS) return hit.value;
    ...
    const value: PublishedValues = row
      ? { version: Number(row.version), publishedAt: iso(row.published_at), entries: valuesOf(row.published as ConfigEntries) }
      : { version: 0, publishedAt: null, entries: {} };
    if (this.valuesCache.size > 10_000) this.valuesCache.clear(); // bounded; games are few, but ids are input
    this.valuesCache.set(gameId, { at: Date.now(), value });
    return value;
  }
  ...
  overwriteDraft(gameId: string, entries: EntryPatch, actor: string, expectedRevision?: number): Promise<ConfigState> {
    return this.writeDraft(gameId, actor, expectedRevision, (s) => {
      ...
      const removed = Object.fromEntries(Object.keys(s.published).filter((k) => !(k in entries)).map((k) => [k, null]));
  ...
  async publish(gameId: string, actor: string, message: string | null, expectedRevision?: number): Promise<{ version: number; state: ConfigState }> {
    return this.tx(async (c) => {
      ...
      for (const [k, e] of Object.entries(s.draft)) {
        const valueChanged = changes[k] !== undefined && !changes[k]!.descriptionOnly;
      ...
      this.valuesCache.delete(gameId);
  ...
  async revisions(gameId: string, limit: number, offset: number): Promise<{ items: RevisionSummary[]; total: number }> {
    const [rows, count] = await Promise.all([
      this.pg.query(
        `SELECT version, published_at, published_by, message, changes FROM game_config_revision
    ...
    return {
      items: rows.rows.map((r) => ({
        ...
        changes: r.changes as Record<string, EntryChange>,
      })),
