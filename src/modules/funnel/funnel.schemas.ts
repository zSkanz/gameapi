...
export const FunnelParams = z.object({
  ...
  funnelName: z.string().regex(FUNNEL_NAME_REGEX),
});
...
const FunnelEvent = z
  .object({
    // Roblox UserId. Past 2^31 already, bounded below 2^53 so a JS Number stays exact.
    playerId: z.number().int().min(1).max(Number.MAX_SAFE_INTEGER),
    step: z.number().int().min(1).max(MAX_FUNNEL_STEP),
...
export const LogBatchBody = z
  .object({
    funnelName: z.string().regex(FUNNEL_NAME_REGEX).default(ONBOARDING_FUNNEL),
