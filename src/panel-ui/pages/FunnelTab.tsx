...
import { Alert, CollapsibleCard, CopyButton, EmptyState, ErrorState, LoadingState, TimeCell, num } from '../ui';
...
export function FunnelTab() {
  ...
  return (
    <div className="stack">
      ...
      <SetupGuide gameId={gameId} />
    </div>
  );
}

/**
 * How to wire a funnel up, with a script to paste. Collapsed by default, like every guide.
 */
function SetupGuide({ gameId }: { gameId: string }) {
  const code = `--!strict
...
return onShopOpened`;
  ...
  return (
    <CollapsibleCard
      title="How to set up a funnel"
      icon={<BookOpen size={16} style={{ color: 'var(--accent)' }} />}
      actions={<CopyButton value={code} label="Copy script" />}
    >
      <Alert kind="info">
